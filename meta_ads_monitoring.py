# FINAL VERSION: Meta Ads Monitoring with Supabase-powered account sync and budget integration
...
async def process_and_save(ad_account_id, time_range, business_name):
    ...
    async with RetryClient(raise_for_status=False, retry_options=retry_options) as client:
        ...
        for insight in insights.get("data", []):
            ad_id = insight.get("ad_id") or "unknown"
            ad_data = ad_map.get(ad_id, {})
            ad_name = ad_data.get("name", "Unknown") or "Unknown"
            ...
            daily_budget = campaign_info.get("daily_budget") or 0
            ...
            if spend > 500:
                flagged = True
                flagged_reason = "High Daily Spend"
            elif cpa > 35:
                flagged = True
                flagged_reason = "High CPA"
            elif leads == 0:
                try:
                    date_obj = datetime.strptime(insight.get("date_start"), "%Y-%m-%d")
                    if (datetime.today() - date_obj).days >= 2:
                        flagged = True
                        flagged_reason = "No leads in 2+ days"
                except:
                    passflagged = True
                flagged_reason = "High CPA"
        ...
        supabase.table("meta_ads_monitoring").insert(rows_to_insert).execute()
...
async def main():
    ...
    AD_ACCOUNTS = get_active_accounts()
